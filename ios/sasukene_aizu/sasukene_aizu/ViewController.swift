...
class ViewController: UIViewController, MKMapViewDelegate, UITextFieldDelegate {
    ...
    // ホスト(必要なホスト名に変えてください)
    var HostName = "http://hiso.dip.jp:3000/"//"http://yousan.sasukene.info:3000/"//"http://localhost:3000/"
...
****/
        // ピンを全部消す
        if let ex : [MKAnnotation] = myMapView.annotations {
...
//        myMapView.removeAnnotation(myMapView.annotations)
    ...
    func csvLoad(filename :String)->[String]{
        ...
        let cvsData = try! NSString(contentsOfFile: csvBundle!, encoding: NSUTF8StringEncoding)
    ...
    func update(){
        if( g_count % 10 == 0 ){
            // ここでNode.jsを叩く
            let val = self.myMapView.centerCoordinate
            let lat = String(stringInterpolationSegment: val.latitude)      //緯度
            let lon = String(stringInterpolationSegment: val.longitude)     //経度
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        let drawview = self.view as AnyObject as! DrawView
        //var drawview = self.view as AnyObject
        let touch = touches.first as UITouch!
        let point = touch.locationInView(drawview as UIView)
        
        // ボタン代わり
        if( ( point.x > 410 ) && ( point.x < 600 ) && ( point.y > 35 ) && ( point.y < 70 ) ){
            print( zipTextField.text, terminator: "" )
            ...
            for( ix = 0 ; ix < zipcodeArray.count; ix++ ){
                ...
                if( csvArray[ 0 ] == zipTextField.text ){
                    ...
                    let myLat: CLLocationDegrees = ( csvArray[ 1 ] as NSString).doubleValue
                    let myLon: CLLocationDegrees = ( csvArray[ 2 ] as NSString).doubleValue
                    let myCoordinate: CLLocationCoordinate2D = CLLocationCoordinate2DMake(myLat, myLon)
